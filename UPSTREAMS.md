...
| [lisa](https://github.com/CodySwannGT/lisa) | Manifest + doctor validation; session rules; sprint-loop → tramat's post-PR loop (CodeRabbit address-or-reject → resolve → merge gate → automerge). Its tiered-template rendering (enforced/seeded/merged) was trialed in M3 and deliberately dropped — tramat uses reference examples + semantic doctor checks instead. | main | 2026-08-14 |
