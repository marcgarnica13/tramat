...
**As a Claude Code plugin (M3):** this repo is a plugin marketplace. Installing it gets you `/tramat:init` (greenfield setup: the agent composes your repo — manifest, DAB skeleton, `src/` helpers with passing tests, CI — from the proven `examples/starter-lakehouse/` starting point, adapted to your answers), `/tramat:doctor` (environment + manifest checks plus semantic convention advisories — run_as pinned to an SP, schedules only in prod, Tier-0 CI present — delegating workspace health to the official Databricks plugin), `/tramat:help`, the `tramat-core` operating rules for any repo with a `tramat.yml`, and a sub-100ms session-start hook that tells the agent where the graph stands. Tramat ships **no renderer and no frozen templates**: the reference example is kept Tier-0 green in CI, and you — with the LLM and the plan — decide what your repo looks like. Every file in your repo is yours; conventions are recorded in `tramat.yml` and checked semantically, never byte-diffed.
